...
compile-assets:
	...
	cd themes/conventional-logs && npm install && npm run build
	echo "Assets compiled"

compile-assets-dev:
	echo "Compiling assets"
	cd themes/conventional-logs && npm install && npm run start &

rebuild-node-sass:
	echo "Rebuilding node sass"
	cd themes/conventional-logs && npm rebuild node-sass
